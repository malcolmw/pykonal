\section{Method}
	...
	\subsection{The Fast Marching Method}
		...
		\begin{algorithm}
			...
			\ForEach{
				...
				Tag node as \textit{Far}\;
			}
			\ForEach{
				$Node\ \in\ \Gamma\left(t=0\right)$
			}{
				Remove \textit{Far} tag from node\;
				Add node to \textit{Close} list\;
				$u_{ijk} \gets 0$\;
			}
			\While{
				\text{\normalfont{length}}$(\textit{Close}) > 0$
			}{
				$Trial \gets\ $node in \textit{Close} with smallest value of $u$\;
				Remove \textit{Trial} node from \textit{Close}\;
				Tag \textit{Trial} node as \textit{Alive}\;
				\ForEach{
					\text{\normalfont Neighbour of} Trial
				}{
					\If{
						\text{\normalfont Neighbour is} \textit{Far}
					}{
						Remove \textit{Far} tag from node\;
						Add node to \textit{Close} list\;
					}
	...
	\subsection{Order of the Finite-Difference Operators}
		...
		Following \citeA{Sethian1999a} we compute values of $u_{ijk}$ using a second-order update when adequate upwind data exists.